...
```{r, eval = FALSE}
luces <- ee$ImageCollection("COPERNICUS/Landcover/100m/Proba-V-C3/Global") %>%
  ee$ImageCollection$map(function(x) x$select("crops-coverfraction")) 
ee_print(luces)
```
...
oe <- GTM %>% full_join(GTM_luces)

plot(
    oe["X2019_crops.coverfraction"], 
       key.pos = 4)
```
