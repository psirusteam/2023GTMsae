...
```{r}
names_cov <- c("depto" , "area" ,  "edad" ,  "anoest", "unida" )
```
...
mrp_cme2 <-
  Aux_Agregado(poststrat = poststrat_df,
             epredmat = epred_mat,
             byMap = c("depto","anoest", "etnia" )) %>% 
  select(-mrp_estimate)

dat_depto2 <- poststrat_df2 %>% group_by(depto,anoest,etnia) %>%
    summarise(
      Benchmarking_estimate = sum(n * pobreza2) / sum(n),
      .groups = "drop"
    )

dat_depto2 <- dat_depto2 %>% left_join(mrp_cme2) %>% 
  mutate(mrp_cv = mrp_estimate_se/Benchmarking_estimate*100) %>% 
  arrange(desc(mrp_cv))
