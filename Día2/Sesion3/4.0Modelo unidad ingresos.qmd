...
knitr::opts_chunk$set(
  ...
  fig.path = "0Recursos/4.0Modelo_unidad_ingreso/"
)
library(printr)
library(kableExtra)
library(tidyverse)
...
library(ggplot2)
library(dplyr)
encuesta_agregada <- readRDS("Data/encuestaGTM14N.rds") %>% 
  group_by(
...
ggplot(data = data.frame(
  ingreso = colMeans(epred_mat),
  n = poststrat_df$n
  )) + 
  geom_histogram(aes(x=ingreso, weights = n)) +
  theme_bw(10)
...
El resultado nos indica que el ingreso medio en el dam 01 es `r round(temp_ing$mrp_estimate,2)` lineas de pobreza
...
El resultado nos indica que el ingreso medio en la dam2 01001 es `r round(temp_ing$mrp_estimate,2)` lineas de pobreza
...
```{r, echo=FALSE, fig.cap = "Ingreso medio por dam2"}
library(sp)
library(sf)
library(tmap)
